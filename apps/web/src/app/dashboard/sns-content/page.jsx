...
import { Zap, Copy, Check, AlertCircle, Instagram, Loader2 } from "lucide-react";
...
const moods = ["따뜻하고 감성적", "트렌디하고 힙하게", "정보 전달 위주", "유머/친근하게"];
    ...
    <div className="max-w-3xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-2">
          <h1 className="text-xl font-semibold text-gray-900 tracking-tight">SNS 콘텐츠 자동 생성</h1>
          <span className="text-xs bg-purple-50 text-purple-600 px-2.5 py-1 rounded-full font-medium">AI 자동화</span>
        </div>
        <p className="text-sm text-gray-500">업종과 오늘의 이벤트를 입력하면 AI가 SNS 게시글과 해시태그를 즉시 완성합니다.</p>
      </div>

      <div className="bg-white border border-gray-200 rounded-xl p-6 mb-5">
        <h2 className="text-sm font-semibold text-gray-900 mb-4">콘텐츠 정보 입력</h2>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-medium text-gray-500 mb-2 block">업종</label>
          ...
          <div>
            <label className="text-xs font-medium text-gray-500 mb-2 block">오늘의 메뉴 · 이벤트 · 할인</label>
            <input
              type="text"
              value={event}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs font-medium text-gray-500 mb-2 block">분위기</label>
              <div className="flex flex-wrap gap-2">
                {moods.map((m) => (
                  <button
                    key={m}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-semibold text-gray-900">생성된 콘텐츠</h2>
              <span className="text-xs border border-gray-200 text-gray-600 px-2 py-0.5 rounded-full">{platform}</span>
            </div>
            <button
