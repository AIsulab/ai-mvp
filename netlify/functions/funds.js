...
export default async function handler(event, context) {
  ...
  try {
    ...
    try {
      ...
      console.warn("공공데이터 API 호출 실패 또는 타임아웃:", e.message);
    }
    ...
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, data: verifiedGrants }),
    };
  } catch (error) {
    return { statusCode: 500, headers, body: JSON.stringify({ success: false, error: "서버 내부 오류가 발생했습니다." }) };
  }
}
